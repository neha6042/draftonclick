                                ...
                                <div class="card-content collapse show">
                                    <div class="card-body card-dashboard">
                                        <table class="table table-striped table-bordered table-responsive file-export">
                                            ...
                                            <tbody>
											<?php $j=1; ?>
											@foreach($arrDepartmentDetails as $key => $val)
												<tr >
													<td>{{$j++}}</td>
													<td>{{$val->bank_name}}</td>
													...
													<td>{{$val->city}}</td>
													<td>{{$val->dept_name}}</td>
													<td>{{$val->dept_manager_name}}</td>
													<td>{{$val->scale_of_dept_manager}}</td>
													<td>{{$val->dept_email}}</td>
													<td>{{$val->dept_alt_email}}</td>
													<td>{{$val->dept_manager_mobile_no}}</td>
													<td>{{$val->dept_manager_contact_no}}</td>
													<td>{{$val->dept_office_address}}</td>
													<td>
														<a class="btn btn-icon btn-success mr-1" href="{{url('/edit_department/'.base64_encode($val->id))}}" title="Edit Department"><i class="ft-edit"></i></a>

														<button type="button" class="btn btn-icon btn-success mr-1" data-toggle="modal" data-placement="top" data-target = "#myModal_del"  title="Delete Department" onclick ="  $('#delete_value').val({{$val->id}}); "><i class="ft-delete"></i></button>
			  ...
			  <div class="modal fade" id="myModal_del" role="dialog">
				<div class="modal-dialog">
				  ...
				  <div class="modal-content">
						 <form name="forms_del" action="{{url('delete_department')}}" method="POST"> 
